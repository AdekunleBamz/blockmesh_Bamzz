...
install_node() {
    ...
    echo -e "${CYAN}https://app.blockmesh.xyz/register?invite_code=Bamzz${RESET}"
    echo -ne "${YELLOW}Have you completed registration? (y/n): ${RESET}"
    read registered

    if [[ "$registered" != "y" && "$registered" != "Y" ]]; then
        echo -e "${RED}Please complete the registration and use referral code Bamzz to continue.${RESET}"
